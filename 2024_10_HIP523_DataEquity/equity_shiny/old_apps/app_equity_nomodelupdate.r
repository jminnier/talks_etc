...
server <- function(input, output, session) {
  ...
  example1_models <- reactive({
    data <- example1_data()
    list(
      full = lm(funded ~ women_owned + earnings, data = data),
      bivariate = lm(funded ~ women_owned, data = data)
    )
  })
  ...
  example2_models <- reactive({
    data <- example2_data()
    list(
      full = lm(funded ~ black_owned + sole_prop, data = data),
      bivariate = lm(funded ~ black_owned, data = data)
    )
  })
  ...
  example3_models <- reactive({
    data <- example3_data()
    list(
      uncorrelated = lm(funded ~ black_owned + priority_zone, data = data),
      all = lm(funded ~ black_owned + sole_prop + priority_zone, data = data)
    )
  })
