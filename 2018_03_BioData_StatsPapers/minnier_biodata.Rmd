...
- Not if there's correlation in the data, or confounder(s)
- What if the data is matched and you want to know if the variables move in the same direction/correlate? 
...
### Rule 7: <font style="color: #EB811B;">Provide Assessments of Variability</font>
  - Almost all measurements have uncertainty
  - We CAN estimate it!
...
# Example: Parametric vs Non-parametric Tests?

### <font style="color: #EB811B;">Often misunderstood</font>
...
iris %>%sample_n(40)%>% ggplot(aes(x=Species,y=Petal.Length))+
  geom_boxplot(outlier.size = 0)+geom_jitter(width=0.1,aes(color=Species),size=2)
```
